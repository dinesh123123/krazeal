...
const ordersummery=async(req,res)=>{
    ...
    try{
        if(cartId){
            const raju=await Order_summery.findOne({"userId":userId,"cartId":cartId});
            if(raju){
...
const AddCart_api=async(req,res)=>{
    const {userId,productId,gst,delevery_charge,prices,color,size,duration}=req.body;
...
if(userId && productId ){
    const product=new Cart({userId,productId,gst,delevery_charge,prices,color,size,duration});
    
    const result=await product.save();
    res.status(200).json({result:"true",message:"data add sucessfully",data:result});
}else{
    res.status(400).send({result:"false",message:"required parameters are userId,productId,gst,delevery_charge,duration"});
  }
}
    }catch(error){
        res.status(400).send({result:"false",message:"get some error", msg:error.message});
